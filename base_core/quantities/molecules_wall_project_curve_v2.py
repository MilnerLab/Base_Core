...
class SpinnabilityCurve:
    ...
    B_used_hz: np.ndarray
    D_used_hz: np.ndarray
    used_gasphase_mask: np.ndarray
    wall_time_s: float | None
    wall_centrifuge_frequency_hz: float | None
    wall_raman_frequency_hz: float | None
    wall_J_cont: float | None
...
def _centrifugal_wall_level_frequency(
    B_hz: float,
    D_hz: float,
) -> tuple[float | None, float | None]:
    """
    Continuous centrifugal-wall estimate for the slope-based local rotational
    frequency

        f_rot(J) = (2J + 1) [B - 2 D J(J+1)].

    Returns
    -------
    tuple[float | None, float | None]
        (J_wall_cont, f_wall_hz) where J_wall_cont is the continuous J at which
        f_rot(J) is maximal, and f_wall_hz is that maximal mechanical rotation
        frequency. Returns (None, None) if no finite wall exists.
    """
    if B_hz <= 0.0:
        raise ValueError("B_hz must be > 0.")
    if D_hz < 0.0:
    ...
    radicand = (2.0 * B_hz / D_hz + 1.0) / 3.0
    if radicand <= 0.0:
        return None, None

    J_wall = 0.5 * (-1.0 + np.sqrt(radicand))
    if J_wall < 0.0:
        return None, None

    f_wall = (2.0 * J_wall + 1.0) * (B_hz - 2.0 * D_hz * J_wall * (J_wall + 1.0))
    if f_wall <= 0.0:
        return None, None

    return float(J_wall), float(f_wall)




def _select_projected_gasphase_branch(
    ...
    """
    Project the last droplet dressed state onto the instantaneous gas-phase
    eigenbasis and choose the strongest projected component that still yields a
    positive averaged effective inverse inertia.
    """
    ...
    for idx in order:
        ...
        populations = np.abs(candidate) ** 2
    ...
    # Fallback: if all candidates fail this criterion, just return the dominant
    # overlap component.
    return eigenvectors[:, int(order[0])]
...
class Molecule:
    ...
    def calculate_spinnability_curve(
        ...
        *,
        ...
        switch_to_gasphase_after_wall: bool = True,
        project_to_gasphase_at_wall: bool = True,
    ) -> SpinnabilityCurve:
        """
        ...
        - optional phenomenological switch from droplet constants to gas-phase
          constants once the droplet centrifugal wall is reached in the current
          slope-based picture
        - at the first post-wall time step, optionally project the droplet
          dressed state onto the gas-phase instantaneous eigenbasis and continue
          from the dominant gas-phase component
        ...
        droplet_B_hz = float(self.droplet.B)
        droplet_D_hz = float(self.droplet.D)
        if droplet_B_hz <= 0.0:
            raise ValueError("B must be > 0.")
        if droplet_D_hz < 0.0:
            raise ValueError("D must be >= 0.")

        gasphase_B_hz = droplet_B_hz
        gasphase_D_hz = 0.0
        if self.gasphase.B is not None:
            gasphase_B_hz = float(self.gasphase.B)
        if self.gasphase.D is not None:
            gasphase_D_hz = float(self.gasphase.D)
        if gasphase_B_hz <= 0.0:
            raise ValueError("gasphase B must be > 0 if provided.")
        if gasphase_D_hz < 0.0:
            raise ValueError("gasphase D must be >= 0 if provided.")

        wall_J_cont, wall_centrifuge_frequency_hz = _centrifugal_wall_level_frequency(
            droplet_B_hz,
            droplet_D_hz,
        )
        if wall_centrifuge_frequency_hz is None:
            wall_time_s = None
            wall_raman_frequency_hz = None
        else:
            wall_time_s = 2.0 * pi * wall_centrifuge_frequency_hz / angular_acceleration_rad_per_s2
            wall_raman_frequency_hz = 2.0 * wall_centrifuge_frequency_hz
        ...
        B_used_values = np.full(n_points, np.nan, dtype=float)
        D_used_values = np.full(n_points, np.nan, dtype=float)
        used_gasphase_mask = np.zeros(n_points, dtype=bool)

        previous_vector = state_vector.copy()
        previous_use_gasphase = False
        ...
        for i in range(n_points):
            ...
            use_gasphase = (
                switch_to_gasphase_after_wall
                and wall_time_s is not None
                and time_s[i] >= wall_time_s
            )

            current_B_hz = gasphase_B_hz if use_gasphase else droplet_B_hz
            current_D_hz = gasphase_D_hz if use_gasphase else droplet_D_hz

            B_used_values[i] = current_B_hz
            D_used_values[i] = current_D_hz
            used_gasphase_mask[i] = use_gasphase

            h0_diag_hz = current_B_hz * jj1_diag - current_D_hz * jj1_diag_sq
            ...
            entering_gasphase = use_gasphase and (not previous_use_gasphase)
            if entering_gasphase and project_to_gasphase_at_wall:
                # The Hamiltonian changes discontinuously here. Instead of
                # continuing the droplet branch through that jump, project the
                # last droplet dressed state onto the instantaneous gas-phase
                # eigenbasis and pick the dominant physically usable component.
                current_vector = _select_projected_gasphase_branch(
            ...
            previous_use_gasphase = use_gasphase
            ...
            eff_inverse_I_by_J = current_B_hz - 2.0 * current_D_hz * jj1_by_J
            eff_inverse_I = float(np.dot(p_J, eff_inverse_I_by_J))
        ...
        return SpinnabilityCurve(
            ...
            B_used_hz=B_used_values,
            D_used_hz=D_used_values,
            used_gasphase_mask=used_gasphase_mask,
            wall_time_s=wall_time_s,
            wall_centrifuge_frequency_hz=wall_centrifuge_frequency_hz,
            wall_raman_frequency_hz=wall_raman_frequency_hz,
            wall_J_cont=wall_J_cont,
        )
