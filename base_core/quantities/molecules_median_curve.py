...
class Molecule:
    ...
    def calculate_spinnability_curve(
        ...
        *,
        ...
        - effective inverse inertia taken from the weighted median J via
              B_eff = B - 2 D J_med (J_med + 1)
        ...
        SpinnabilityCurve
            ...
            <J(J+1)>(t), the weighted median J, the modal J, and the old
            J_eff reconstructed from <J(J+1)>.
        """
        if angular_acceleration_rad_per_s2 <= 0.0:
            raise ValueError("angular_acceleration_rad_per_s2 must be > 0.")
        if ramp_duration_s <= 0.0:
        ...
        for i in range(n_points):
            ...
            median_jj1 = median_J * (median_J + 1.0)
            eff_inverse_I = B_hz - 2.0 * D_hz * median_jj1
            if eff_inverse_I <= 0.0:
                continue
